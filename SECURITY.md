...
Only the latest release receives fixes. There are no maintained release branches, so a security fix
...
This file is the policy, covering how to report something and what is supported. The reasoning
behind the guardrails themselves is documented alongside the code it protects, so a contributor can
find *why* a check exists rather than only that it does:
