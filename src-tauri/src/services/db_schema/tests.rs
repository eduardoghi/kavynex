        ...
        for &(_, ddl) in INDEX_DDLS {
            if index_introduced_in(ddl_object_name(ddl)) <= version {
                sqlx::query(sqlx::AssertSqlSafe(ddl))
                    .execute(pool)
                    .await
                    .unwrap_or_else(|error| panic!("failed to seed {ddl} at v{version}: {error}"));
            }
        }
    }
    ...
    for &(table, ddl) in TRIGGER_DDLS {
        if trigger_introduced_in(table) <= version {
            sqlx::query(sqlx::AssertSqlSafe(ddl))
                .execute(pool)
                .await
                .unwrap_or_else(|error| panic!("failed to seed a trigger at v{version}: {error}"));
        }
    }
...
async fn migrates_a_real_v1_database_to_the_current_schema() {
    ...
    let (progress,): (i64,) = sqlx::query_as("SELECT progress_seconds FROM videos WHERE id = 2")
        .fetch_one(&pool)
        .await
        .unwrap();
...
async fn migration_10_collapses_pre_existing_duplicate_comments_before_indexing() {
    ...
    let (kept_text,): (String,) =
        sqlx::query_as("SELECT text FROM video_comments WHERE video_id = 1 AND comment_id = 'c1'")
            .fetch_one(&pool)
            .await
            .unwrap();
