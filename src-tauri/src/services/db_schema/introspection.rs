//! Read-only schema introspection over `pragma_*` virtual tables, answering whether a table has a
//! given column, a UNIQUE index on an exact column list, or an `ON DELETE CASCADE` foreign key.
//! Used both
