...
|---|---|---|---|
| `utils/path.rs`, `library/guard.rs`, `yt_dlp/url.rs`, `binaries.rs` | initial, `guard.rs` re-measured 2026-09-02 | `guard.rs` share compare: 12 mutants, 11 caught, 1 excluded as equivalent (the `\|\|` entry below), in 6min together with the `cookies.rs` probe, 17 tested and 17 caught | The four the gate started with: path containment, the library-path guard, the yt-dlp host allow-list, external-binary resolution. The weekly run of 2026-08-31 found one survivor in `guard.rs` that had been there since the share compare was written. Weakening `requested_share == configured_share` to `true` passed, because a different share is refused either way: past the compare, both UNC paths fail to canonicalize on the runner and the string fallback says no. What the mutant changed is *which* branch refuses, and on Windows that is refusing up front versus authenticating to the hostile host over SMB first. Nothing the return value shows can tell those apart, so the compare is its own function now, `network_paths_name_the_same_share`, asserted directly in both directions. The lesson is the one the `commands/security.rs` row already carries. A guard whose refusal is indistinguishable from a later fallback's is untested however many cases reach it. |
...
| `services/yt_dlp/cookies.rs` | 2026-07-27, re-measured 2026-09-02 | 11 mutants, 11 caught, 0 missed; the header probe added since then measured 7 mutants, 6 caught, 1 excluded as equivalent | The `.txt` gate, the network-path refusal, and the browser allow-list deciding which cookie store yt-dlp may read. Clean. The Netscape-header probe that joined it later brought the third copy of the capped-read shape, and with it the `<` to `<=` survivor the two readers in `utils/io.rs` and `process.rs` already had excluded, for the same reason (see below). |
...
| `services/media_protocol.rs` | 2026-09-20 | 96 mutants, 18 unviable, 67 caught, 11 missed on the first pass; 4 missed after, all excluded below | The custom scheme that serves library media, added in v1.6.0 because Tauri's asset protocol truncates range responses. It parses a `Range` header and percent-decodes a path, both arriving from the webview, which is the reason it was worth gating rather than the reason it was skipped at first. The first pass found a real defect, and not in the parsing. The compile-time floor on `MAX_RANGE_BYTES` was anchored to the value measured to *fail* (Tauri's `1000 * 1024`) rather than the smallest measured to play, so it accepted `(16 + 1024) * 1024`, four percent over the broken cap and four times under 4 MiB. A guard admitting a value that reproduces the bug it guards against is not one. The rest were coverage gaps, starting with a percent escape truncated at the end of the string, a single-byte `bytes=100-100` range sitting on the inverted-range boundary, eight content-type arms whose loss silently turns a playable file into `application/octet-stream`, and `empty_response`, where a body-less 200 would read to the media element as a zero-length file that loaded fine. Of the 11 that survived that pass, 7 were inside `handle` and were extracted rather than excluded, into `status_for_open_error` (404 for a file deleted outside the app, 500 for this process failing) and `byte_count` (the count sizes the body while the pair sizes `Content-Range`, so disagreeing by one is corrupt rather than short). The 4 left are `handle` itself. On 2026-09-23 the file turned out to have been killing CI runners since it joined, which a local pass on Windows could not show. `percent_decode` advanced an index with `+= 1` and `+= 3`, and the mutants `*= 1` and `-= 3` stall that index, so the loop pushes the same bytes into its output forever. On Windows the test process aborts on allocation failure and cargo-mutants records a catch; on the ubuntu runner the OOM killer takes the runner, which is the loss in #55. It was rewritten to walk the input by slice patterns, so every pass consumes at least a byte by construction and no mutation can stall it. That removed the hazard rather than excluding it, and took `percent_decode` from 21 mutants to 10, all caught. The gated scope went to 1764. |
...
| `replace read_lossy_line(_capped)? -> Option<String> with Some("xyzzy".into())` | Not a hang, and worse than one, which is why it went unexplained for weeks while its empty-string sibling above sat excluded. `"xyzzy"` is neither empty nor JSON, so `read_capped_json_stdout` pushes it into `log_lines` on every pass of the same loop, without awaiting any I/O. Memory is gone in seconds, inside the 300-second deadline, and the OOM takes the CI runner with it. That is the "runner has received a shutdown signal" in #55, since shards 3 and 5 died on these two mutants, at position 147 of each slice, in every run from the day the scope reached 1780. Found by running the shards with `--caught --unviable` so the last finished mutant was logged, and confirmed by the index matching the ordered slice exactly. Earlier sampling every 30 seconds had cleared memory as a cause, but it sampled a shard that passed and could not see a spike that fast. |
...
| `replace handle -> Response<Vec<u8>>` | `handle` takes a `UriSchemeContext`, which only Tauri constructs while dispatching a request, so the function cannot be called from a unit test at all. Everything it decides was pulled out into functions that can be, and all of those stay in scope, meaning `percent_decode`, `parse_byte_range`, `clamp_range`, `media_content_type`, `status_for_open_error`, `byte_count` and `empty_response`. What the exclusion covers is the assembly. |
| `delete ! in handle` | The one concession here worth naming rather than burying, because it is the `!` on the authorization check, and the mutant serves every path the scope refuses. It is excluded for reach rather than for importance, since inverting it needs a live scope on a live handle. The guard is `asset_protocol_scope().is_allowed`, which this module calls precisely so there is no second implementation to test. The set it consults is the one `register_library_asset_scope` grants and revokes against the persisted library path, and `commands/security.rs` is in scope and tested for exactly that. **If that delegation ever becomes a local reimplementation, this entry must go with it.** |
