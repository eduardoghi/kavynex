...
| `replace != with == in migrate_directory_contents` | Gates only an `eprintln` diagnostic during migration. |
| `replace match guard !ext.trim().is_empty() with true in alternative_destination_path` | Differs from `true` only when the extension is an empty string, which `Path::extension` never yields (a trailing-dot name has no extension). The `with false` / `delete !` variants are killed by the migration extension test. |
