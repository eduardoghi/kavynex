...
Kavynex has one maintainer, so this document is deliberately short. A long enforcement ladder
with committees and appeal stages would describe a project that does not exist here, and a rule
nobody could actually apply is worse than a plain statement of the standard.

The standard itself is not unusual, and it aligns with the
[Contributor Covenant](https://www.contributor-covenant.org/version/2/1/code_of_conduct/), which
is worth reading if you want the fuller version.
...
- **Disagree about the technical question.** "This breaks the containment check because X" is a
  contribution. "This is amateur work" is not, and it also says nothing anyone can act on.
- **Assume the other person is trying.** A bug report light on detail is usually someone who does
  not know what detail helps, not someone wasting your time. `SUPPORT.md` exists to tell them.
...
it can be reported to GitHub as well, which can act in ways a single repository maintainer cannot.
